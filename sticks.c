    ...
    case WS_CANCEL:
        {
            ...
            if (isupper(monster = (char) mvwinch(mw, y, x))) {
                ...
                if (obj->o_which == WS_POLYMORPH) {
                    ...
                    if (omonst != monster)
                        msg("The monster twists and changes!");
                    ...
                    if (tp->t_pos.y != y || tp->t_pos.x != x)
                        tp->t_oldch =
                            (char) mvwinch(cw, tp->t_pos.y, tp->t_pos.x);
                }
            }
        }
        break;
    ...
    case WS_SLOW_M:
        ...
        if (isupper(mvwinch(mw, y, x))) {
            ...
            if (obj->o_which == WS_HASTE_M) {
                msg("The %s starts moving faster!",
            ...
            for (y = 0; y < BOLT_LENGTH && !used; y++) {
                ...
                default:
                    if (!bounced && isupper(ch)) {
                        if (!save_throw
                            (VS_MAGIC,
                             (struct thing *) ldata(find_mons(unc(pos))))) {
                            bolt.o_pos = pos;
